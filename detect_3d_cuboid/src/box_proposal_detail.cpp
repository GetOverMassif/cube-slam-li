									 ...
									 MatrixXd all_lines_raw, std::vector<ObjectSet> &all_object_cuboids)
{
	...
	for (int object_id = 0; object_id < num_2d_objs; object_id++)
	{
		...
		ca::Profiler::tictoc("Two cuboid detection");
		...
		for (int sample_down_expan_id = 0; sample_down_expan_id < down_expand_sample_all.size(); sample_down_expan_id++)
		{
			...
			// printf("rect = (%d,%d,%d,%d)\n",left_x_expan_distmap, top_y_expan_distmap, width_expan_distmap, height_expan_distmap);
            cv::Mat im_canny;
    ...
    clock_t time1 = clock();
    std::cout << time1 << "\n\n";
    std::cout << "time consumption : " << 1000 * (time1 - time0) / (double)CLOCKS_PER_SEC << "ms\n\n";
